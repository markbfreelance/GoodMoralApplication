  ...
  <div x-data="{ sidebarOpen: false }" class="flex min-h-screen">
    ...
    <main class="flex-1 p-4 bg-gray-100 min-h-screen">
      ...
      <form method="GET" action="{{ route('sec_osaSearch') }}" class="mb-4 p-2 bg-white shadow-md rounded-lg">
        ...
        <a href="{{ route('sec_osa.major') }}" class="mt-4 inline-block bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">Clear</a>
      </form>
          ...
          <table class="min-w-full bg-white border border-gray-200 shadow-md rounded-lg">
            ...
            <tbody>
              ...
              <tr class="border-b hover:bg-gray-100">
                ...
                <td class="py-3 px-6">
                  @if ($student->status == 0)
                  <form action="{{ route('sec_osa.document', $student->id) }}" method="POST" enctype="multipart/form-data" class="mt-2">
                    @csrf
