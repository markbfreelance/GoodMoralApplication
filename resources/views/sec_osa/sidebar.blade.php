...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-64'"
  ...
  <nav>
    ...
    <a href="{{ route('sec_osa.minor') }}"
      class="gap-2 h-20 items-center flex px-4 py-2 {{ request()->routeIs('sec_osa.minor') ? 'bg-gray-800 text-spupGold' : 'text-white hover:bg-gray-800 hover:text-spupGold' }}">
    ...
    <a href="{{ route('sec_osa.major') }}"
      class="gap-2 h-20 items-center flex px-4 py-2 {{ request()->routeIs('sec_osa.major') ? 'bg-gray-800 text-spupGold' : 'text-white hover:bg-gray-800 hover:text-spupGold' }}">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
        stroke="currentColor" class="size-10">
        <path stroke-linecap="round" stroke-linejoin="round"
