<x-app-layout>
    ...
    <div x-data="{ sidebarOpen: false }" class="flex">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-64'"
            ...
            <nav class="mt-4">
                <a href="{{ route('registrar.dashboard') }}" class="block px-4 py-2 hover:bg-gray-700">Dashboard</a>
                <a href="{{ route('registrar.psgApplication') }}" class="block px-4 py-2 hover:bg-gray-700 
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <div class="mb-8">
                    <nav class="flex space-x-8 items-center">
                        <!-- Pending Button -->
                        <a href="{{ route('registrar.psgApplication', ['status' => 'pending']) }}" class="px-6 py-3 bg-blue-600 text-white font-semibold text-sm rounded-md shadow-sm transition-all duration-300 ease-in-out hover:bg-blue-700 focus:ring-2 focus:ring-blue-400 focus:outline-none 
       {{ request()->get('status') == 'pending' ? 'bg-blue-700 border-b-4 border-blue-800' : '' }}">
                            Pending
                        </a>
                        <a href="{{ route('registrar.psgApplication', ['status' => 'pendingAtAdmin']) }}" class="px-6 py-3 bg-yellow-600 text-white font-semibold text-sm rounded-md shadow-sm transition-all duration-300 ease-in-out hover:bg-yellow-700 focus:ring-2 focus:ring-yellow-400 focus:outline-none 
       {{ request()->get('status') == 'approved' ? 'bg-yellow-700 border-b-4 border-yellow-800' : '' }}">
                            Admin Approval
                        </a>
                        <!-- Approved Button -->
                        <a href="{{ route('registrar.psgApplication', ['status' => 'approved']) }}" class="px-6 py-3 bg-green-600 text-white font-semibold text-sm rounded-md shadow-sm transition-all duration-300 ease-in-out hover:bg-green-700 focus:ring-2 focus:ring-green-400 focus:outline-none 
       {{ request()->get('status') == 'approved' ? 'bg-green-700 border-b-4 border-green-800' : '' }}">
                            Approved
                        </a>
                        <!-- Rejected Button -->
                        <a href="{{ route('registrar.psgApplication', ['status' => 'rejected']) }}" class="px-6 py-3 bg-red-600 text-white font-semibold text-sm rounded-md shadow-sm transition-all duration-300 ease-in-out hover:bg-red-700 focus:ring-2 focus:ring-red-400 focus:outline-none 
