<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center space-x-4">
      <img src="https://placehold.co/40x40" alt="Admin Picture" class="w-10 h-10 rounded-md object-cover">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-64'"
      ...
      <nav>
        ...
        <a href="{{ route('admin.psgApplication') }}" class="gap-2 h-20 items-center flex px-4 py-2 text-gray-700 hover:bg-gray-700 hover:text-gray-300">
          <x-icon-logout class="h-10 w-10 text-gray-700 hover:text-red-600" />
        ...
        <form method="POST" action="{{ route('logout') }}">
          @csrf
          <a href="{{ route('logout') }}"
            class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:text-red-600"
